...
/******************* MASTER PROCESSOR WORKS HERE******************************************************/ 
	...
	double start = time(NULL);
   	if(rank==MASTER)
  	{
		...
		initialize(Data.centroids,num_clusters);
		resetData(former_clusters,num_points);
		resetData(latter_clusters,num_points);

		const MPI_Datatype data[]={MPI_INT, MPI_INT, MPI_POINT};
		int blocklenData[]={2, 2, num_clusters};
		MPI_Aint disp=0;
		MPI_Type_create_struct(1,blocklenData,&disp,data,&MPI_DATA);
		MPI_Type_commit(&MPI_DATA);
		...
		{
			printf("Sending to [%d]\n",dex);
			MPI_Send(&Data						,1				, MPI_DATA       ,dex,0,MPI_COMM_WORLD);
...
/*************END OF MASTER PROCESSOR'S BRANCH -- SLAVE PROCESSORS' JOB IS TO FOLLOW ************************/
	else
	{
		...
		MPI_Recv(&Data    	,1           ,MPI_DATA  ,MASTER,0,MPI_COMM_WORLD,&status);
		...
		while(flag == 1)
		{
			...
			{
				slave_clusters[dex]=whoIsYourDaddy(received_points[dex],Data.centroids,num_clusters);
			}
